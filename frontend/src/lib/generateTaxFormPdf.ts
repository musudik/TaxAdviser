  ...
  try {
    ...
    if (formData.incomeInfo) {
      ...
      if (income.investments?.bankCertificate?.length > 0) {
        income.investments.bankCertificate.forEach(
      ...
      if (income.foreignIncomeTaxCertificateFile?.length > 0) {
        income.foreignIncomeTaxCertificateFile.forEach(
          (file: any, index: number) => {
            if (file && file.name) {
              files.push({
    ...
    if (formData.expenses) {
      ...
      const workRelatedExpenses =
        expenses.workRelatedExpenses || formData.workRelatedExpenses;
      if (workRelatedExpenses) {
        // Business trip proofs
        if (workRelatedExpenses.businessTripsCosts?.proof?.length > 0) {
          workRelatedExpenses.businessTripsCosts.proof.forEach(
            (file: any, index: number) => {
              if (file && file.name) {
        ...
        if (workRelatedExpenses.workEquipment?.expenses) {
          workRelatedExpenses.workEquipment.expenses.forEach(
            (expense: any, expIndex: number) => {
      ...
      if (expenses.specialExpenses) {
        ...
        if (specialExpenses.insurance?.expenses) {
          specialExpenses.insurance.expenses.forEach(
            (expense: any, expIndex: number) => {
              if (expense.file && expense.file.length > 0) {
                expense.file.forEach((file: any, fileIndex: number) => {
                  if (file && file.name) {
        ...
        if (specialExpenses.donations?.expenses) {
          specialExpenses.donations.expenses.forEach(
            (expense: any, expIndex: number) => {
              if (expense.file && expense.file.length > 0) {
                expense.file.forEach((file: any, fileIndex: number) => {
                  if (file && file.name) {
        ...
        if (specialExpenses.professionalDevelopment?.expenses) {
          specialExpenses.professionalDevelopment.expenses.forEach(
            (expense: any, expIndex: number) => {
              if (expense.file && expense.file.length > 0) {
                expense.file.forEach((file: any, fileIndex: number) => {
                  if (file && file.name) {
      ...
      if (expenses.extraordinaryBurdens) {
        ...
        if (extraBurdens.medicalExpenses?.expenses) {
          extraBurdens.medicalExpenses.expenses.forEach(
            (expense: any, expIndex: number) => {
              if (expense.file && expense.file.length > 0) {
                expense.file.forEach((file: any, fileIndex: number) => {
                  if (file && file.name) {
      ...
      const promises = batch.map(async ({ path, file, fieldPath }) => {
        try {
          ...
          try {
            ...
            console.log(
              `File uploaded successfully, but download URL cannot be retrieved due to permission restrictions: ${path}`,
            );
...
const uploadPdfToFirebase = async (
  ...
  try {
    ...
    try {
      ...
      console.log(
        `PDF uploaded successfully, but download URL cannot be retrieved due to permission restrictions: ${pdfPath}`,
      );
      return null;
    }
  } catch (error) {
...
//   const splitGermanLabel = doc.splitTextToSize(
//     germanLabel || "Label (DE)",
//     actualWidth,
//   );
...
//   const splitSelectedLabel = doc.splitTextToSize(
//     selectedLabel || "Label",
//     actualWidth,
//   );
...
//   doc: PDF,
//   yPositions: { col1: number; col2: number },
//   columnState: { nextCol: number },
//   pageHeight: number,
//   bottomMargin: number,
//   germanTitle: string,
...
const addPageNumber = (doc: PDF) => {
  ...
  doc.text(`${doc.getNumberOfPages()}`, pageWidth / 2, pageHeight - 10, {
    align: "center",
  });
  doc.setTextColor(0); // Reset to black
};
...
export const generateTaxFormPdf = async (
  formData: any,
  germanI18nData: any,
  i18nData: any,
): Promise<string | null> => {
  try {
    ...
    //   if (yPositions[`col${colIndex}`] + 50 > pageHeight - bottomMargin) {
    //     // 50 is a buffer height
    //     doc.addPage();
    ...
    //   key: string,
    //   defaultValue: string,
    // ): { german: string; selected: string } => {
    //   const germanValue =
    //     germanI18nData?.taxForm?.[section]?.[key] || defaultValue;
    ...
    doc.setFontSize(14);
    doc.text(
      `${fullName} - ${formData.applicationId || ""}`,
      pageWidth / 2,
      margin + 20,
      { align: "center" },
    );
    ...
    doc.text(`Datum / Date: ${submissionDate}`, pageWidth / 2, margin + 30, {
      align: "center",
    });
