...
export class TaxFormService {
  ...
  async create(createTaxFormDto: CreateTaxFormDto): Promise<TaxFormResponse> {
    try {
      ...
      // Extract firstName, lastName, and submissionYear for database columns
      const personalInfo = createTaxFormDto.personalInfo || {};
      const taxYear = createTaxFormDto.taxYear || {};
      
      // Extract key fields for searching
      const firstName = personalInfo.firstName?.toString() || null;
      const lastName = personalInfo.lastName?.toString() || null;
      const submissionYear = taxYear.year?.toString() || null;
      ...
      const taxFormData: any = {
        ...
        // Store extracted fields for search
        firstName,
        lastName,
        submissionYear,
        // Store JSON data
        ...
        taxYear: createTaxFormDto.taxYear || {},
        signature: createTaxFormDto.signature || {},
        language: createTaxFormDto.language || 'en',
      };

      // Store businessInfo in businessExpenses since businessInfo is not in the schema
      if (createTaxFormDto.businessInfo) {
        taxFormData.businessExpenses = {
          ...taxFormData.businessExpenses,
          businessInfo: createTaxFormDto.businessInfo
        };
      }
  ...
  async update(id: string, updateTaxFormDto: UpdateTaxFormDto): Promise<TaxFormResponse> {
    try {
      ...
      // Extract key fields for searching if present
      const updateFields: any = {};
      
      if (updateTaxFormDto.personalInfo) {
        if (updateTaxFormDto.personalInfo.firstName) {
          updateFields.firstName = updateTaxFormDto.personalInfo.firstName.toString();
        }
        if (updateTaxFormDto.personalInfo.lastName) {
          updateFields.lastName = updateTaxFormDto.personalInfo.lastName.toString();
        }
      }
      
      if (updateTaxFormDto.taxYear && updateTaxFormDto.taxYear.year) {
        updateFields.submissionYear = updateTaxFormDto.taxYear.year.toString();
      }
      ...
      const updateData: any = { 
        ...
        userId: updateTaxFormDto.userId,
        currentStep: updateTaxFormDto.currentStep,
        status: updateTaxFormDto.status,
        personalInfo: updateTaxFormDto.personalInfo,
        incomeInfo: updateTaxFormDto.incomeInfo,
        rentalIncome: updateTaxFormDto.rentalIncome,
        foreignIncome: updateTaxFormDto.foreignIncome,
        ...
        signature: updateTaxFormDto.signature,
        taxYear: updateTaxFormDto.taxYear,
        language: updateTaxFormDto.language,
        submittedAt: updateTaxFormDto.submittedAt,
      };
      ...
      // Handle businessInfo - store it in businessExpenses
      if (updateTaxFormDto.businessInfo) {
        if (!updateData.businessExpenses) {
          // Get current businessExpenses first
          const currentForm = await this.prisma.taxForm.findUnique({
            where: { id },
            select: { businessExpenses: true }
          });
          
          updateData.businessExpenses = Object.assign({}, 
            currentForm?.businessExpenses || {}, 
            { businessInfo: updateTaxFormDto.businessInfo }
          );
        } else {
          updateData.businessExpenses = Object.assign({},
            typeof updateData.businessExpenses === 'object' ? updateData.businessExpenses : {}, 
            { businessInfo: updateTaxFormDto.businessInfo }
          );
        }
      }
      
      // If businessExpenses exist, add it to the update data
      if (updateTaxFormDto.businessExpenses) {
        updateData.businessExpenses = updateTaxFormDto.businessExpenses;
      }
  ...
  private mapToTaxFormResponse(data: any): TaxFormResponse {
    ...
    // Extract businessInfo from businessExpenses if it exists
    const businessInfo = businessExpenses.businessInfo || {};
    ...
    return {
      ...
      businessInfo: businessInfo as JsonSection, // Set from businessExpenses.businessInfo
